...
- **Customer accounts ("customer numbers")** — a `customers` table with one
  number per company (`C-0001`, `C-0002`, …). The number is generated by the
  backend (`nextCustomerNumber`) and never accepted from the client, so it can
  never be reused or forged.
